...
const SOURCES = [
  { title: 'Lok Sabha & Rajya Sabha', desc: 'Official session records, attendance registers, Q&A transcripts, and member participation logs.', icon: Building2, accent: 'bg-orange-500/10 border-orange-500/20 text-orange-500' },
...
const SCORE_BREAKDOWN = [
  ...
  { label: 'Questions Asked', weight: 25, desc: 'Holding the government accountable during Question Hour.', color: 'bg-orange-500' },
  { label: 'Bills Sponsored', weight: 20, desc: 'Drafting and introducing Private Member Bills.', color: 'bg-green-600' },
  ...
  return (
    <div className="flex-1 w-full bg-background pb-24 min-h-screen">
      ...
      <header className="max-w-4xl mx-auto px-4 pt-10 pb-16 text-center relative">
        <div className="absolute inset-x-0 top-0 h-64 bg-gradient-to-b from-orange-500/8 to-transparent -z-10 rounded-b-[3rem]" />
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-20">
        ...
        <section>
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-black mb-3 flex items-center justify-center gap-2">
              <HelpCircle className="w-7 h-7 text-orange-500" /> Frequently Asked Questions
            </h2>
          </div>
          <div className="space-y-3 max-w-3xl mx-auto">
            {FAQS.map((faq, i) => (
              <div key={i} className={cn('bg-card border rounded-2xl overflow-hidden transition-colors duration-200', openFaq === i ? 'border-orange-500/30' : 'border-border/60 hover:border-orange-500/20')}>
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full flex items-center justify-between p-5 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
        ...
        <section className="pt-8 border-t border-border/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-2xl mx-auto">
            {[
              { href: '/citizen/search', icon: SearchCheck, label: 'Find Your MP', color: 'bg-orange-500/10 text-orange-500' },
              { href: '/citizen/compare', icon: ArrowRightLeft, label: 'Compare MPs', color: 'bg-green-600/10 text-green-600' },
              { href: '/citizen', icon: MapPin, label: 'Explore India Map', color: 'bg-pink-500/10 text-pink-500' },
            ].map(item => (
              <Link key={item.href} href={item.href} className="p-5 bg-card border border-border/60 rounded-2xl hover:border-orange-500/30 hover:shadow-sm transition-all group text-center flex flex-col items-center gap-3">
