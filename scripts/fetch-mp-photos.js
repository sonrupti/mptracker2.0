...
function loadSansadDetails() {
  ...
  const filtered = rows
    .filter(r => {
      if (!houseCol) return true;
      const normalized = String(r[houseCol]).toLowerCase().replace(/[^a-z]/g, '');
      return normalized.includes('loksabha') || normalized === 'ls';
    })
    .map(r => ({ name: String(r[nameCol]).trim(), mpCode: String(r[codeCol]).trim() }))
    .filter(r => r.name && r.mpCode);

  if (filtered.length === 0 && houseCol) {
    const distinctValues = [...new Set(rows.map(r => String(r[houseCol])))].slice(0, 10);
    console.log(`   ⚠️  0 rows matched the house filter. Distinct "${houseCol}" values found:`, distinctValues);
  }
